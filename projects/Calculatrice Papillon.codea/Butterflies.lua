...
function Butterfly:init(ms,c)
    ...
    local xc,yc = WIDTH/2,(HEIGHT/2+HEIGHT/8+HEIGHT/15)+((HEIGHT-(HEIGHT/2+HEIGHT/8+HEIGHT/15))/2)
    local shape = {}
    x = (rnd()-0.5)*WIDTH*0.95 + xc
    y = (rnd()-0.5)*(HEIGHT-(HEIGHT/2+HEIGHT/8+HEIGHT/15))*0.9 + yc
    z = 0
    w = (rnd()+2)*4
    h = w
...
function Butterfly:fly(type,t,mult)
    ...
    if type then
        ...
        if self.y<(HEIGHT/2+HEIGHT/8+HEIGHT/15) then self.y =
        self.y + (HEIGHT-(HEIGHT/2+HEIGHT/8+HEIGHT/15)) end
        if self.y>HEIGHT then self.y = self.y - (HEIGHT-(HEIGHT/
        2+HEIGHT/8+HEIGHT/15)) end
